...
|-----|------|---------|-------------|
| admin | object | `{"pullPolicy":"IfNotPresent","repository":"ghcr.io/intersectmbo/cardano-node:10.3.1","tag":"1.35.4"}` | The admin pod is a special pod. This pod is air-gapped (nothing in, nothing out) and mounts cold keys from a Vault. Use this pod for admin operations such as KES key signature and node certificate signature |
...
| image.repository | string | `"ghcr.io/intersectmbo/cardano-node:10.3.1"` |  |
| image.tag | string | `"1.35.4"` | Overrides the image tag whose default is the chart appVersion. See [here](https://hub.docker.com/r/ghcr.io/intersectmbo/cardano-node:10.3.1/tags?page=1&ordering=last_updated) the full list of tags. |
